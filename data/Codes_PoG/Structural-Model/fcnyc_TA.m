...
elseif varying_et == 2
    for j=1:T
        ...
        if j==1
            ...
            difcd(1,1)=phi^2*time_multiplier(j)*zt(1)+2*psi^2*et(1)+time_multiplier(j)*varcsi;
        elseif j<T
            %difcd(j,j)=phi^2*zt(min(max(j-1,1),T_z))+(ind_et(j)-ind_et(j-1))*psi^2*et(min(j,T_e))+(ind_et(j)-ind_et(j-1))*varcsi;
            difcd(j,j)=phi^2*time_multiplier(j)*zt(min(max(j-1,1),T_z))+(ind_et(j)-ind_et(j-1))*psi^2*et(min(j,T_e))+(ind_et(j)-ind_et(j-1))*varcsi;
        else
            %difcd(j,j)=phi^2*1.5*zt(T_z)+(ind_et(j)-ind_et(j-1))*psi^2*et(min(j,T_e))+(ind_et(j)-ind_et(j-1))*varcsi;
            difcd(j,j)=phi^2*time_multiplier(j)*zt(T_z)+(ind_et(j)-ind_et(j-1))*psi^2*et(min(j,T_e))+(ind_et(j)-ind_et(j-1))*varcsi;
        end
        difcme(j,j)=varv(min(j,T_u))+varv(min(max(j-1,1),T_u));
        if j>=2
            difcme(j-1,j)=-varv(min(j-1,T_u));
...
elseif varying_et == 2
    for j=1:T
        ...
        if j<T
            ...
            %difcy(j,j)=(0.5)*phi*time_multiplier(j) *zt(min(max(j-1,1),T_z))+psi*et(min(j,T_e)); %this 0.5 is dependent on the fact that there was one year intervals, need to redo integrals for 2/3 year intervals.
            difcy(j,j)=(time_multiplier(j)-0.5)*phi*zt(min(max(j-1,1),T_z)) +psi * et(min(j,T_e))
        else
            %difcy(T,T)=phi*1.5*zt(T_z)+psi*(1+teta)*et(T_e);
            %difcy(T,T)=(0.5)*phi*time_multiplier(j) *zt(T_z)+psi*(1+teta)*et(T_e);
            difcy(T,T)=(time_multiplier(j)-0.5)*phi*zt(T_z)+psi*et(T_e);
        ...
        if j>=2
            ...
            difcy(j-1,j)=(0.5)*phi*zt(min(max(j-1,1),T_z)) -psi*et(min(j-1,T_e));
        else
        end
    end
elseif varying_et == 3
