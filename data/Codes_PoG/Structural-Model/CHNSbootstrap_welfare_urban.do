	...
	* CREATE PANEL DATASET FOR MATLAB
	...
	if `TA'== 1{
		* RUN THE MATLAB CODE THAT ESTIMATES THE MODEL
		shell "$mymatlab" -wait -nodisplay -nosplash -nodesktop -r < Mainresults_urban_TA.m Mainresults_urban_TA
		...
		* IMPORT ESTIMATES FROM MATLAB
		...
		shell "$mymatlab" -wait -nodisplay -nosplash -nodesktop -r < Mainresults_urban.m Mainresults_urban
...
* BOOTSTRAPPING no TA
*************************************************************************** 
use "Baseline_Data\dydc_spec2.dta", clear

gen   double bootid = case_id
tsset bootid interview_year

bootstrap, /*noisily*/ nodrop cluster(case_id) idcluster(bootid) reps(50) seed(200) saving(boot_spec2_final,replace): progbootouter bootid
estimates save est_spec2_final, replace

* CREATE A MATRIX OF ESTIMATES FOR MATLAB (TO BE USED IN MAINRESULTS_WELFAESP.M, WELFARE COUNTERFACTUALS ACROSS SPACE)
use "boot_spec2_final.dta", clear
export delimited _b_r1 _b_r2 _b_r3 _b_r4 _b_r5 _b_r6 _b_r7 _b_r8 _b_r9 _b_r10 _b_r11 _b_r12 _b_r13 _b_r14 _b_r15 _b_r16 using "boot_spec2_final4matlab.csv", novarnames replace

*boot_spec2_final4matlab.csv is the bootstrapped estimates. 
*Point estimates with all the data are stored in est_spec2_final
...
bootstrap, /*noisily*/ nodrop cluster(case_id) idcluster(bootid) reps(50) seed(200) saving(boot_spec2_final_TA,replace): progbootouter_TA bootid
estimates save est_spec2_final_TA, replace

* CREATE A MATRIX OF ESTIMATES FOR MATLAB (TO BE USED IN MAINRESULTS_WELFAESP.M, WELFARE COUNTERFACTUALS ACROSS SPACE)
use "boot_spec2_final_TA.dta", clear
export delimited _b_r1 _b_r2 _b_r3 _b_r4 _b_r5 _b_r6 _b_r7 _b_r8 _b_r9 _b_r10 _b_r11 _b_r12 _b_r13 _b_r14 _b_r15 _b_r16 using "boot_spec2_final4matlab_TA.csv", novarnames replace
