...
*************************************************************************** 
 forvalues nspec = 1/7	{
	use "Extra_Data\dydc_spec`nspec'.dta", clear
	
	gen   double bootid = case_id
	tsset bootid interview_year

	bootstrap, /*noisily*/ nodrop cluster(case_id) idcluster(bootid) reps(50) seed(200) saving(boot_extra`nspec'_final,replace): progbootouter bootid
	estimates save est_extra`nspec'_final, replace
