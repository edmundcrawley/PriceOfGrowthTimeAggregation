...
program define progbootinner, rclass
	...
	* RUN THE MATLAB CODE THAT ESTIMATES THE MODEL
	shell "$mymatlab" -wait -nodisplay -nosplash -nodesktop -r < Mainresults_boot_imp_TA.m Mainresults_boot_imp_TA
...
*************************************************************************** 
 forvalues nspec = 0/4	{
	use "Imputed_Data\dydc_imp`nspec'.dta", clear
	
	gen   double bootid = case_id
	tsset bootid interview_year

	bootstrap, /*noisily*/ nodrop cluster(case_id) idcluster(bootid) reps(2) seed(200) saving(boot_imp`nspec'_final,replace): progbootouter bootid
	estimates save est_imp`nspec'_final, replace
	}
